...
public class ZipHelper {
    ...
    public static void doCompress(ArrayList<byte[]> byteList, ArrayList<String> nameList, ZipOutputStream out) throws IOException {

        for (int i=0;i<byteList.size();i++){
            out.putNextEntry(new ZipEntry(nameList.get(i)));
            out.write(byteList.get(i),0,byteList.get(i).length);
            out.closeEntry();
        }
    }
